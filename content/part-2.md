...
1. **`try` / `except` / `finally`** ensures `disconnect(terminate=True)` always runs once `connect()` succeeded. A kiosk disconnect becomes an immediate graceful terminate upstream.
...
Impact: Competitive noise when economics look mysterious even if transcripts are crisp. Preempt with lifecycle clarity defaults.
```
